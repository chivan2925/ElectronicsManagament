...
function renderField(field, values, errors, onChange) {
  ...
  if (field.type === "checkbox") {
    return (
      <label className="inline-flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5">
        <input
  ...
  return (
    <form className={cn("space-y-5", className)} onSubmit={handleSubmit}>
      <div className={cn("grid gap-4", gridClass)}>
        {fields.map((field) => (
          <label className={cn("space-y-1.5", field.fullWidth && "md:col-span-2")} key={field.name}>
            {field.type !== "checkbox" ? (
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
        ...
        <button
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-black text-white transition hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-70"
          disabled={loading}
          type="submit"
        >
