  ...
  return (
    ...
    >
      <div
        className={
          isAdminSurface
            ? "flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-[0_18px_45px_rgba(15,23,42,0.12)]"
            : "flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4 shadow-[0_0_40px_rgba(0,91,255,0.14)]"
        }
      >
        <Loader2 className={isAdminSurface ? "animate-spin text-blue-600" : "animate-spin text-blue-300"} size={20} />
        <span className={isAdminSurface ? "text-sm font-bold text-slate-700" : "text-sm font-bold text-slate-200"}>
          {message}
        </span>
      </div>
    </div>
