        ...
        <Card className="relative overflow-hidden p-6 md:p-8 lg:p-10" variant="glass">
          ...
          <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-center">
            <div>
              ...
              <h1 className="max-w-3xl text-4xl font-black leading-tight text-white md:text-5xl">{title}</h1>
              <p className="mt-4 max-w-2xl text-base font-medium leading-relaxed text-slate-300 md:text-lg">{subtitle}</p>

              {features.length > 0 && (
                <ul className="mt-7 grid gap-3 sm:grid-cols-2">
                  {features.map((feature) => (
                    <li className="flex items-center gap-3 text-sm font-semibold text-slate-300" key={feature}>
                      <CheckCircle2 className="text-emerald-300 drop-shadow-[0_0_12px_rgba(110,231,183,0.45)]" size={18} />
                      {feature}
                    </li>
                  ))}
                </ul>
