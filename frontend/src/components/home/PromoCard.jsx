...
function PromoCard({ promo }) {
  return (
    <MotionArticle className={`premium-transition group relative isolate overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${promo.gradient} p-4 shadow-xl shadow-black/24 backdrop-blur-xl hover:-translate-y-1 hover:border-blue-300/60 hover:shadow-[0_0_38px_rgba(0,91,255,0.26),0_24px_60px_rgba(0,0,0,0.34)]`} variants={{ ...fadeUp, hover: hoverGlow }} whileHover="hover">
      ...
      <div className="relative z-10 flex items-center gap-4">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="mt-1 text-lg font-black leading-tight text-white drop-shadow-[0_0_18px_rgba(255,255,255,0.12)]">{promo.title}</h3>
          <p className="mt-1 text-sm font-bold text-blue-100">{promo.discount}</p>
          <a className="premium-transition mt-4 inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-sm font-black text-white shadow-inner shadow-white/[0.03] hover:border-blue-200/50 group-hover:text-blue-100" href="/">
            Mua ngay
        ...
        <div className="relative shrink-0 overflow-hidden rounded-xl border border-white/10 bg-slate-950/35 p-1.5 shadow-[0_0_28px_rgba(0,91,255,0.12)]">
          ...
          <MotionImg alt={promo.title} className="premium-transition relative z-10 h-24 w-28 rounded-lg object-cover ring-1 ring-white/10 group-hover:ring-blue-300/40" src={promo.image} variants={{ hover: imageZoom }} />
        </div>
      </div>
    </MotionArticle>
  );
