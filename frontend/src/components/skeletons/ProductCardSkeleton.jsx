...
function ProductCardSkeleton({ variant = "default" }) {
  ...
  return (
    <article className="skeleton-card h-full overflow-hidden rounded-2xl p-3 sm:p-4">
      ...
      <div className="mt-4 space-y-3">
        ...
        <div className="rounded-2xl border border-blue-200/10 bg-white/[0.03] p-2.5 sm:p-3">
          <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-end sm:justify-between sm:gap-3">
            <div className="space-y-2">
              <SkeletonBlock className="h-5 w-28 rounded-full" />
              <SkeletonBlock className="h-3 w-20 rounded-full" />
            </div>
            <SkeletonBlock className="hidden h-6 w-12 rounded-full sm:block" />
          </div>
